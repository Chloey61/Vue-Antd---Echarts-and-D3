(function(){
	...
	b.prototype = {
		init:function(){
			...
			t.top = t.t.offset().top - $('#leftBox').scrollTop();
			t.left = t.t.offset().left - $('#leftBox').scrollLeft();
			t.height = t.t.outerHeight();
			t.width = t.t.outerWidth();
      t.w_width = $('#leftBox').width();
      t.w_height = $('#leftBox').height();
		...
		move:function(){
			...
			t.c.on('mousedown',function(e){
				...
				t.height = t.t.outerHeight();
				t.width = t.t.outerWidth();
				let h = $('#leftBox').offset().top + 35;
				let w = $('#leftBox').offset().left + 1;
				$(document).on('mousemove',function(e){
					t.left2 = t.left+e.pageX-t.x-w;
					t.top2 = t.top+e.pageY-t.y-h;
					if(t.left2 <=0){
						t.left2 = 0;
					...
					if(t.top2 <=0){
						t.top2 = 0;
					}else if(t.top2 >= t.w_height-t.height-35){
					  // title heigth 34
						t.top2 = t.w_height-t.height -35;
					}
		...
		size:function(){
			...
			t.t.append('<span class="bg_change_size" style="background-color: #ddd"></span>');
